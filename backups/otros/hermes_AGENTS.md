...
## Git Auto-Commit & Verification Workflow (Local Infrastructure)

Para garantizar la seguridad del código frente a reinicios del agente y mantener un historial de Git limpio, el entorno local intercepta tus herramientas en tiempo de ejecución:

1. **Commits temporales (WIP):** Cada escritura exitosa (`write`, `edit`, `apply_patch`) genera automáticamente un commit local del tipo `wip(auto): [tool] [archivo]` sin ejecutar hooks de Git (para velocidad).
2. **Consolidación (Squash):** No intentes hacer commits manuales para tus cambios intermedios.
3. **Trigger de Éxito:** Al terminar tus tareas, debés ejecutar la suite de pruebas (ej. corriendo `pytest`). Si los tests pasan con éxito (código de salida 0), la infraestructura local ejecutará automáticamente un `git reset --soft` y consolidará todos tus commits `wip(auto)` en un único commit definitivo bajo el estándar de **Conventional Commits** (ej: `feat(auto): update auth.ts`).
