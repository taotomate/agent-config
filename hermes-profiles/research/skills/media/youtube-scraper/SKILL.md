...
This skill (`media/youtube-scraper`) and `data-science/youtube-scraper-pipeline` cover the same project.
This skill is the **canonical** reference — it has the complete schema, full pitfall list, and migration details.
The pipeline skill is a leaner summary focused on the workflow steps.
When updating, update this skill first; pipeline should stay a concise subset.
