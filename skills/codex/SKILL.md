...
OAuth from `build-mcp-server\references\auth.md` after `hermes auth add openai-codex`. For the
standalone Codex CLI, a valid CLI OAuth session may live under
`build-mcp-server\references\auth.md`; do not treat a missing `OPENAI_API_KEY` alone as proof
