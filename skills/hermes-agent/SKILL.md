...
- Triggers: "hermes-agent", "use hermes-agent"
...
To use Hermes Agent with Telegram, you need to set up the gateway. This involves:
1. Installing and configuring the Hermes Gateway.
2. Setting up your Telegram bot.
3. Configuring the gateway to connect to Telegram.
...
This will guide you through the setup process, including creating a new bot on Telegram and configuring it with Hermes Agent.

Once configured, you can interact with Hermes Agent via Telegram. The agent will be able to respond to your messages, execute commands, and perform tasks as needed.

For more information about setting up the gateway, refer to the [Gateway documentation](https://hermes-agent.nousresearch.com/docs/user-guide/messaging/).
...
/reload-skills       Re-scan agent-customization\references\skills.md for added/removed skills
...
build-mcp-server\references\auth.md         OAuth tokens and credential pools
...
- **Telemetry:** sidecar at `agent-customization\references\skills.md.usage.json` holds
...
**User Feedback**: The user confirmed that Hermes Agent is already installed and working on Windows 10, but noted that there are multiple directories scattered across different locations. This indicates that the default installation structure might not be as streamlined as expected for a 'portable' setup. However, this is normal — the multi-directory structure is intentional to separate configuration, logs, sessions, skills, etc., which enhances maintainability and scalability.

**Recommendation**: For users seeking a more compact or portable setup, they can create a symbolic link or use a custom script to consolidate frequently accessed directories into a single location. This would not affect the functionality but could simplify navigation for users who prefer minimal directory sprawl.
...
### Destructive Git Operations on Windows

**`git reset --hard` is blocked by the terminal safety policy.** Even when the user explicitly confirms (`"si"` / `"yes"`), the command returns `BLOCKED: User denied this command`. This is a persistent security constraint — not a transient error.

**Do NOT retry the same command or attempt workarounds.** Instead, tell the user to run destructive git commands directly in their terminal:
...
This applies to: `git reset --hard`, `git branch -D`, `git checkout -b` from detached state, and similar destructive operations.

**Safe pattern for updates with local changes:**
1. Create a backup branch: `git checkout -b backup-local && git checkout main`
2. Tell user: `git reset --hard v2026.6.19` (they run it)
3. Verify result, then reapply local customizations from the backup branch
4. Optionally: restore stash if one was created before the reset
