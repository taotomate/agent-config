...
- Triggers: "llm-wiki", "use llm-wiki"
...
- Needs Telegram integration for delivering content (as demonstrated in this session)
...
### New Pitfall: Tool Installation Failures
- When a tool fails to install due to network or binary issues, capture the exact fix (install command, config step) under an existing setup skill rather than marking 'this tool does not work' as a permanent constraint. This ensures future sessions can handle similar failures with the same fix.
  mark in frontmatter, flag for user review.
