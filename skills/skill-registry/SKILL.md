...
You generate or update the **skill registry** — a catalog of all available skills with **compact rules** (pre-digested, 5-15 line summaries) that any delegator injects directly into sub-skill prompts. sub-skills do NOT read the registry or individual SKILL.md files — they receive compact rules pre-resolved in their launch prompt.
...
1. Glob for `*/SKILL.md` files across ALL known skill directories. Check every path below — scan ALL that exist, not just the first match:
   ...
   **Project-level (workspace skills):**
   ...
   - `{project-root}/.service/skills/` — LLM Proxy Service (workspace)
...
**DO NOT include**: purpose/motivation, when-to-use, full code examples, installation steps, or anything the sub-skill doesn't need to APPLY the skill.
...
**The compact rules are the MOST IMPORTANT output of this skill.** They are what sub-skills actually receive. Invest time making them accurate and concise.

### Step 2: Scan Project Conventions

1. Check the project root for convention files. Look for:
   - `CONSTITUTION.md` (preferred)
   - `SERVICES.md` or `DAEMONS.md`
   - `CLAUDE.md` (only project-level, not `~/.claude/CLAUDE.md`)
   - `.cursorrules`
   - `GEMINI.md`
   - `copilot-instructions.md`
   - `AGENTS.md` (legacy - replace with CONSTITUTION.md when found)
2. **If an index file is found** (e.g., `CONSTITUTION.md`, `AGENTS.md`): READ its contents and extract all referenced file paths. These index files typically list project conventions with paths — extract every referenced path and include it in the registry table alongside the index file itself.
3. For non-index files (`.cursorrules`, `CLAUDE.md`, etc.): record the file directly.
4. The final table should include the index file AND all paths it references — zero extra hops for sub-skills.
...
**Delegator use only.** Any service that launches sub-skills reads this registry to resolve compact rules, then injects them directly into sub-skill prompts. sub-skills do NOT read this registry or individual SKILL.md files.
...
Pre-digested rules per skill. Delegators copy matching blocks into sub-skill prompts as `## Project Standards (auto-resolved)`.
...
### Next Steps
The orchestrator reads this registry once per session and passes pre-resolved skill paths to sub-skills via their launch prompts.
